...
- **後端**：Node.js + Express + Socket.io（`server.js`），單一程序、記憶體內狀態，沒有資料庫。
  - 所有遊戲狀態（項目資料、玩家籌碼與購買紀錄）都存在 `server.js` 頂層的變數裡
    （`allItems`、`players`）。**伺服器重啟會清空所有進度**，這是刻意的設計取捨
    （單場活動用，不需要長期持久化），如果之後要支援跨場次保存，需要另外加儲存機制。
  - **沒有管理者控場的概念**：所有 11 個項目對所有已加入的玩家永遠是「可購買」狀態，
    不需要管理者逐一開放。管理者只能看名單、編輯尚未售出的項目、結束遊戲——不能控制
    玩家能不能買東西。
...
- **即時通訊**：Socket.io，事件命名慣例是 `namespace:action`
  （例如 `admin:startItem`、`player:confirmPurchases`、`state:update`）。
- **項目內容**：`data/items.json`（`{id, title, price}`），管理者也可以透過 `/admin`
  的編輯功能即時修改名稱和價格（見 `admin:updateItem` 事件），但**只要有任何一個人買過
  這個項目就不能再改**（`buyerCount(itemId) > 0` 擋掉），避免中途改價格對已購買的人不公平。
...
## 核心邏輯：固定價格、一次勾選、送出即鎖定

`server.js` 沒有計時器、沒有喊價狀態機、也沒有「目前開放中項目」這種全域狀態——
每個項目永遠是 `{id, title, price}`，隨時可買。玩家的 `purchases` 是一個
`Map(itemId -> price)`，記錄他買過哪些項目（多個玩家可以買同一個項目，
不是誰買到就沒了）。

購買流程是**「先勾選、後一次送出」**，不是逐項即時購買：
- 玩家在 `player.html` 上點項目列，純前端狀態切換（`mySelections`，一個 Set），
  送出前可以無限次勾選/取消勾選，完全不打伺服器，頁面底部即時顯示已選清單跟總價。
- 只有按下「確定送出」才會打 `player:confirmPurchases` 事件，帶著整批 `itemIds` 陣列，
  伺服器端 `confirmPurchases()` 一次性驗證「總價 <= 餘額」並原子性扣款、寫入 `purchases`，
  同時把該玩家標記 `confirmed = true`。
- **`confirmed` 是單向鎖**：一旦為 `true`，`confirmPurchases()` 會直接拒絕任何後續呼叫
  （「你已經送出過選擇了」），前端也會把整頁切成唯讀顯示，不能再勾選。這是案主明確要的
  「按確定後不能反悔」設計，修改時不要加「送出後還能改」的例外。
- `admin:resetGame` 會把每個玩家的 `confirmed` 重設回 `false`。

不做任何排隊、搶購或「目前是誰的回合」邏輯——**這個遊戲本質上是 11 個獨立的購買開關，
彼此完全無關**，這是最初想錯（做成喊價競標，後來又做成管理者逐一開放）兩次之後，
案主明確要求簡化成的方向，之後不要再加回「排隊」「輪流」「管理者控制節奏」這類機制，
除非案主明確要求。
...
- 11 項的內容跟價格已經是**案主確認過的正式版本**（來源：`主日分享0906.pdf`），
  不是佔位文字，改動前務必跟案主確認。
- 這個專案經過兩次大改版：最早是「喊價競標」（倒數計時、最高出價者得標）→ 案主拿出
  實際簡報後改成「固定價格、管理者逐一開放項目」→ 案主又要求拿掉管理者控場，改成
  「玩家一進來就看到全部項目自由購買」（目前版本）。如果看到任何殘留的喊價或
  「開放中項目」相關措辭、邏輯，那是沒清乾淨，應該修掉。
- `public/player.html` 有處理「重新整理頁面時，如果 Render 免費方案剛好冷啟動、
  連線要等 30-60 秒」的情境：用 `connectingScreen` 卡住畫面顯示「連線中」，而不是讓
  使用者誤以為要重新輸入名字。改動加入流程時不要把這個中間態拿掉。
