...
`item:buy` 事件驗證只有三件事：項目存在、玩家還沒買過這項、餘額夠付。
不夠這三個條件就直接拒絕，不做任何排隊、搶購或「目前是誰的回合」邏輯——
**這個遊戲本質上是 11 個獨立的購買開關，彼此完全無關**，這是最初想錯（做成喊價競標，
後來又做成管理者逐一開放）兩次之後，案主明確要求簡化成的最終版本，之後不要再加回
「排隊」「輪流」「管理者控制節奏」這類機制，除非案主明確要求。
